<template>
  <v-container>
    <v-row>
      <v-col cols="12">
        <h1 class="text-center">Ongoing Matters</h1>
      </v-col>
      <v-col cols="12">
        <v-text-field
          variant="outlined"
      ...
      <v-col cols="12">
        <h1 class="text-center">Completed Matters</h1>
      </v-col>
      <v-col cols="12">
        <v-table>
          <thead>
...
const rules = {
  required: (value) => {
    return Boolean(value) || 'Field is Required'
  },
  length: (value) => {
    return value.length >= 3 || 'Must be more than three characters'
  }
}
